...
export const AllFurnituresCard = ({ obj, path }: AllFurnituresCardProp) => {
  const { id, img, title, desc, price } = obj;
  ...
  const handleSaveProduct = (evt: React.ChangeEvent<HTMLInputElement>) => {
    if (evt.currentTarget.checked) {
      ...
      setSavedData(deletedProduct);
      localStorage.setItem('all-products', JSON.stringify(deletedProduct));
    }
  };
